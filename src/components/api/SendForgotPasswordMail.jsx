...
const SendForgotPasswordMail = ({email, setShowErrorMessage, setShowLoadingMessage, setShowSuccessMessage}) => {
    const [result, setResult] = useState({ message: null, statusCode: null });

    const fetchData = async () => {
        const apiEndpoint = process.env.REACT_APP_apiHost + "/user/forgotPassword";
        const data = {
            "email": email
        };
        try {
