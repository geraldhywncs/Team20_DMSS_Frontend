...
function GetCategorySelection({
  ...
  useEffect(() => {
    try {
      const apiEndpoint = REACT_APP_apiHost + "/category/readCategory";
...
function addCategory(data, callback, setCategoryAdded) {
  const apiEndpoint = REACT_APP_apiHost + "/category/addCategory";
  callApi(apiEndpoint, "POST", data)
    .then((response) => {
      callback(response);
      if (response.status_code === "200") {
...
function deleteCategory(data, callback) {
  const apiEndpoint = REACT_APP_apiHost + "/category/deleteCategory";
  callApi(apiEndpoint, "POST", data)
    .then((response) => {
      callback(response);
    })
